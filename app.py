...
players_df = pl.read_csv('data/top_goalscorers.csv')
@app.route('/')
def index():
...
def search_player():
    ...
    player_name_clean = player_name.strip().replace('♦', '').replace('#', '').lower()

    df = pl.read_csv('data/top_goalscorers.csv')

    player_data = df.filter(
        df['Player'].str.to_lowercase().str.contains(player_name_clean)
    )

    if player_data.height > 0:
        player_info = player_data.select(['Rank', 'Player', 'Goals scored', 'Matches played', 'Goals per match [nb 1]']).to_dicts()
        result = [{"Rank" :player_info[0]['Rank'],
                   "Player": player_name, "Goals scored": player_info[0]['Goals scored'], 
                   "Matches played": player_info[0]['Matches played'], 
                   "Goals per match": player_info[0]['Goals per match [nb 1]']}]
        return render_template('result5.html', player_info=result)
    else:
...
def quiz2():
    questions = [
        {"question": "Who scored the most goals in World Cup history?", "answer": "Miroslav Klose"},
        {"question": "Who won the 2022 World Cup Golden Boot?", "answer": "Kylian Mbappe"},
        {"question": "Who won the 2018 World Cup Golden Ball?", "answer": "Luka Modric"},
        {"question": "Who scored the most goals in a single World Cup tournament?", "answer": "Just Fontaine"},
        {"question": "Which player has the most goals in World Cup history for Brazil?", "answer": "Ronaldo"}
    ]
    ...
    if request.method == 'POST':
        ...
        for i, q in enumerate(questions):
            ...
            if user_answer and user_answer in q['answer'].lower():  # Check if the user's input is part match the correct answer
                score += 1
        return render_template('quiz_result2.html', score=score, total=len(questions))
